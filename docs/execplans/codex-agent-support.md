...
- [x] Run focused validation commands. Full workspace validation that requires
  OPA was initially blocked in this local environment because `opa` was not on
  `PATH`.
- [x] (2026-06-29 21:38Z) Investigated live Codex hook failures after OPA was
  installed. Reproduced the failure as path resolution before policy
...
- Observation: The global Codex hook command generated by
  `cupcake init --global --harness codex` pointed `--policy-dir` at
  `~/.config/cupcake`, but `ProjectPaths` only recognised project roots and
  directories literally named `.cupcake`.
  Evidence: Running the generated command with a Codex `PreToolUse` fixture
  made the engine look for
  `/home/leynos/.config/.cupcake/policies/codex` and exit before invoking OPA.
  Running the patched binary resolves policies at
  `/home/leynos/.config/cupcake/policies/codex` and emits
...
Post-completion live hook debugging found that global Codex hooks failed before
OPA because the engine treated `~/.config/cupcake` as a project root instead of
as a Cupcake config root. The resolver now recognises direct config roots by
